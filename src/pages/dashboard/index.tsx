import React, {useState} from 'react';
import DashboardTable from "~/component/dashboard_table/dashboard_table";
import NavBar from "~/navbar/navbar";
import Footer from "~/component/footer/footer";

const Dashboard = () => {


    const [isFirstDivOpen, setIsFirstDivOpen] = useState(false);
    const [isSecondDivOpen, setIsSecondDivOpen] = useState(false);
    const [isThirdDivOpen, setIsThirdDivOpen] = useState(false);

    const handleSendMoneyButtonClick = () => {
        setIsFirstDivOpen(!isFirstDivOpen);
    };
    const handleMycashTransferClicked = () => {
        setIsFirstDivOpen(!isFirstDivOpen);
        setIsSecondDivOpen(!isSecondDivOpen);
    }
    const handleRecipientClicked = () => {
        setIsFirstDivOpen(!isFirstDivOpen);
        setIsSecondDivOpen(!isSecondDivOpen);
        setIsThirdDivOpen(!isThirdDivOpen);
    }
    return (
        <>
            <NavBar isAuthenticated={true}/>
            <div
                className="relative text-white h-[450px] bg-cover bg-no-repeat"
                style={{
                    backgroundImage: "url('/assets/images/digicel_hackathon_image.png')",
                    backgroundBlendMode: 'multiply',
                    backgroundColor: 'rgba(255, 0, 0, 0.5)'  // This represents the red overlay with opacity
                }}
            >
                <div className="absolute inset-0 flex flex-col justify-center pl-6">
                    <h1 className="text-4xl font-bold">My Wallet ®</h1>
                    <p className="text-6xl font-semibold mt-2">1,234,567.00</p>
                </div>
            </div>

            <div className="bg-white py-4 px-6  mt-2">
                {/* Heading */}
                <h2 className="text-xl font-semibold mb-8">Choose a transaction</h2>

                {/* Buttons Container */}
                <div className="flex space-x-4">
                    {/* Send Button */}
                    <button
                        onClick={handleSendMoneyButtonClick}
                        className="flex items-center border-2 border-red-500 text-red-500 rounded-lg px-20 py-5 space-x-2">
                        <div>
                            <img
                                src="https://media.istockphoto.com/id/942191652/vector/fast-cash-icon.jpg?s=612x612&w=0&k=20&c=30voLsjQwUFUHoffU1z44-nEUC_Rbg1cJcM1WP2fALA="
                                alt="Send Icon" className="w-5 h-5"/>
                        </div>
                        <span className="text-lg">Send</span> {/* Larger text size */}
                    </button>

                    {/* Receive Button */}
                    <button
                        className="flex items-center border-2 border-red-500 text-red-500 rounded-lg px-20 py-5 space-x-2">
                        <div>
                            <img
                                src="https://cdn4.iconfinder.com/data/icons/money-filled-outline/64/money-colored-15-512.png"
                                alt="Send Icon" className="w-5 h-5"/>
                        </div>
                        <span className="text-lg">Receive</span> {/* Larger text size */}
                    </button>

                    {/* Pay Bills Button */}
                    <button
                        className="flex items-center border-2 border-red-500 text-red-500 rounded-lg px-20 py-5 space-x-2">
                        <div>
                            <img
                                src="data:image/png;base64,"
                                alt="Send Icon" className="w-5 h-5"/>
                        </div>
                        <span className="text-lg">Pay Bills</span> {/* Larger text size */}
                    </button>
                </div>
            </div>
            <DashboardTable/>

            {
                isFirstDivOpen && (
                    <div className="fixed top-0 right-0 bg-white w-1/4 h-full">
                        <div className="p-10">
                            <img src="assets/images/Frame104.png" className="cursor-pointer"></img>
                            <p className="text-red-500 font-hurme mt-8">Send Money</p>
                            <p className="font-hurme font-semibold text-[25px] mt-4">Select Transfer Method</p>
                            <img onClick={handleMycashTransferClicked} src="assets/images/Frame102.png"
                                 className="mt-6 cursor-pointer"></img>
                        </div>
                    </div>
                )
            }

            {
                isSecondDivOpen && (
                    <div className="fixed top-0 right-0 bg-white w-1/4 h-full">
                        <div className="p-10">
                            <img src="assets/images/Frame104.png" className="cursor-pointer"></img>
                            <p className="text-red-500 font-hurme mt-8">Send Money</p>
                            <p className="font-hurme font-semibold text-[25px] mt-4">Choose Recipient</p>
                            <p className="text-red-500 font-hurme text-right">+ Add Recipient</p>
                            <img onClick={handleRecipientClicked} src="assets/images/Group6.png"
                                 className="mt-6 cursor-pointer"></img>
                            <img onClick={handleRecipientClicked} src="assets/images/Group6.png"
                                 className="mt-6 cursor-pointer"></img>
                            <img onClick={handleRecipientClicked} src="assets/images/Group6.png"
                                 className="mt-6 cursor-pointer"></img>
                        </div>
                    </div>
                )
            }


            {
                isThirdDivOpen && (<div className="fixed top-0 right-0 bg-white w-1/4 h-full">
                    <div className="p-10">
                        <img src="assets/images/Frame104.png" className="cursor-pointer"></img>
                        <p className="text-red-500 font-hurme mt-8">Send Money</p>
                        <p className="font-hurme font-semibold text-[25px] mt-4">Send Money</p>

                        <form className="mt-4">
                            <label htmlFor="default-search"
                                   className="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Search</label>
                            <div className="relative">
                                <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                    <svg className="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                              stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/>
                                    </svg>
                                </div>
                                <input type="search" id="default-search"
                                       className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                       placeholder="J$ 0" required></input>
                            </div>
                        </form>

                        <button className="bg-red-500 w-full h-12 text-white rounded-md p-2 mr-2 mt-6">Proceed</button>

                    </div>
                </div>)
            }

        </>
    );
};

export default Dashboard;
